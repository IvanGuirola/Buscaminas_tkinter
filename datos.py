...
def autenticarUsuario(frame, nick, contrasenya):
    cur.execute(
        "SELECT id, nick, contrasenya, avatar, partides_jugades, partides_guanyades FROM usuarios WHERE nick=? "
        "AND contrasenya=?",
        (nick, contrasenya))
    result = cur.fetchone()
    if result:
...
def mostrarInformacionUsuario(usuario, row, col):
    ...
    botonEliminarUsuario = tk.Button(frameInfo, text="Eliminar usuario",
                                     command=lambda nick=nick: eliminarUsuario(nick))
...
def abrirSubfinestraModificar(nick):
    ...
    botonSeleccionarMod = tk.Button(subventana, text='Seleccionar Imagen',
                                    command=lambda: abrirImagenMod(nuevaNickMod.get()))
    botonSeleccionarMod.grid(row=2, column=1)

    botonActualizarNick = tk.Button(subventana, text='Actualizar Nick',
                                    command=lambda: actualizarNick(nick, nuevaNickMod.get()))
    botonActualizarNick.grid(row=3, column=0, padx=10, pady=10)
    botonActualizarContrasenya = tk.Button(subventana, text='Actualizar Contraseña',
                                           command=lambda: actualizarContrasenya(nick, nuevaContrasenyaMod.get()))
...
def abrirVentanaRegistro():
    ...
    botonSeleccionarAvatar = tk.Button(ventanaRegistro, text="Seleccionar Avatar",
                                       command=lambda: seleccionarAvatar(nuevoNick.get()))
    botonSeleccionarAvatar.grid(row=2, column=1)

    botonRegistrar = tk.Button(ventanaRegistro, text="Registrar",
                               command=lambda: registrarUsuario(nuevoNick.get(), nuevaContrasenya.get(),
                                                                nuevoAvatar.get()))
...
def comprobarInicioPartida():
    if len(frameJugador1.nick.get()) == 0 or len(frameJugador2.nick.get()) == 0:
        ...
        # Obtener la información de ambos jugadores
        jugador1_info = obtener_informacion_usuario(frameJugador1.nick.get())
        jugador2_info = obtener_informacion_usuario(frameJugador2.nick.get())

        if jugador1_info and jugador2_info:
            # Preguntar al usuario cuál jugará la partida
            resultado_jugador = tk.simpledialog.askstring("Selección de Jugador",
                                                          "¿Quién jugará la partida? ({} / {})".format(jugador1_info[1],
                                                                                                       jugador2_info[
                                                                                                           1]))

            if resultado_jugador and resultado_jugador.lower() == jugador1_info[1].lower():
                autenticarUsuario(frameJugador1, frameJugador1.nick.get(), frameJugador1.contrasenya.get())
            elif resultado_jugador and resultado_jugador.lower() == jugador2_info[1].lower():
                autenticarUsuario(frameJugador2, frameJugador2.nick.get(), frameJugador2.contrasenya.get())

            # Después de autenticar al usuario, iniciar el juego
            ejecutar_juego()


# Función para obtener la información de un usuario
def obtener_informacion_usuario(nick):
    cur.execute("SELECT id, nick, contrasenya, avatar, partides_jugades, partides_guanyades FROM usuarios WHERE nick=?",
                (nick,))
    return cur.fetchone()
